...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Received transaction:', { fromAddress, toAddress, amount });
    ...
    */
    ...
    // -----------------------------------------
    // ❌ DISABLE DUPLICATE CHECK & BALANCE CHECK
    // -----------------------------------------
    ...
    */
    ...
    const response = {
      ...
      errors: [],
      warnings: [],
      senderBalance: 999999, // dummy value
      timestamp: new Date().toISOString()
    };

    console.log('Validation bypassed. Returning success:', response);
